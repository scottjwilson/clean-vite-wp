...
function the_theme_detect_vite_server(): array {
    ...
    $client_response = @wp_remote_get($vite_server . '/wp-content/themes/clean-vite/@vite/client', array(
        'timeout' => 1,
        'sslverify' => false,
        'redirection' => 0
    ));
...
function the_theme_output_vite_scripts(): void {
    ...
    if (!$vite['running'] && !the_theme_is_local_environment()) {
        return;
    }
 ...
 */
function the_theme_vite_script_module_type(string $tag, string $handle, string $src): string {
